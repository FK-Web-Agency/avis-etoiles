...
function UserCanNotPlay() {
  return (
    <main className="background-body min-h-full">
      <div className="wrapper">
        <h1 className="h1-bold text-red-700">🕒 Attendez un peu !</h1>
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const { canPlay, setCanPlay, setCurrentAction, setSaveUserHistory } = useGameStore();
  ...
  useEffect(() => {
    ...
    if (userHistory?.lastGamePlayed) {
      ...
      isEqual(today, userHistory.lastGamePlayed) ? setCanPlay(false) : setCanPlay(true);
    }

    // Verify the action the user has to do
    if (userHistory?.actions) {
      const actions = userHistory?.actions;
      if (actions.includes('google')) setCurrentAction('instagram');
      if (actions.includes('instagram')) setCurrentAction('facebook');
    } else setCurrentAction('google');

    setSaveUserHistory(saveUserHistory);
  }, []);
